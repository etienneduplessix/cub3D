	...
	while (++i < 4)
	{
		if (ft_strncmp(compass_arr[i], mapline_split[0], 3) == 0)
		{
			...
			else
			{
				...
				return (ft_error(map_info, "multiple map texture entries!"));
			}
		}
	}
	free (compass_str);
...
bool	rgb_fill(t_mapInfo	*map_info, char **mapline_split)
{
	...
	if (ft_arrlen((void **)rgb_arr) != 3)
		return (ft_arrfree((void **) mapline_split),
			...
			ft_error (map_info, "Invalid F/C line!"));
	...
	if ((mapline_split[0][0] == 'F' && !map_info->floor_color))
		...
		return (ft_error ((ft_arrfree((void **) mapline_split),
					map_info), "Duplicate F/C lines!"));
	return (ft_arrfree((void **) rgb_arr), true);
}
...
bool	get_info(t_mapInfo	*map_info, char *map_line)
{
	...
	if (!mapline_split)
		return (ft_error (map_info, "error with splitting mapline!"));
	if (ft_arrlen((void **) mapline_split) != 2)
		return (ft_error ((ft_arrfree((void **) mapline_split), map_info),
				"Invalid mapline!"));
	if (ft_strlen(mapline_split[0]) == 2
		&& !ft_strchr(mapline_split[0], '|')
		&& ft_strnstr("NO|SO|WE|EA", mapline_split[0], 11))
	{
		if (textureline_fill(map_info, mapline_split) == false)
			return (false);
	}
		...
		&& (mapline_split[0][0] == 'F' || mapline_split[0][0] == 'C'))
	{
		...
		return (ft_error ((ft_arrfree((void **) mapline_split), map_info),
				"Invalid mapline!"));
	return (ft_arrfree((void **) mapline_split), true);
}
...
int	map_info_fill(t_mapInfo	*map_info, int map_fd)
{
	...
	while (map_line)
	{
		...
		{
			if (is_map_line(map_line) == true)
			{
				if (process_map_arr(map_line, map_info, map_fd) == false)
					return (false);
				map_line = ft_strdup(":)");
			}
			else if (get_info(map_info, map_line) == false)
			{
				while (map_line)
...
t_mapInfo	*get_map(char *map_path)
{
	...
	if (ft_strncmp(&(map_path[ft_strlen(map_path) - ft_strlen(".cub")])
			, ".cub", ft_strlen(".cub")) != 0)
		return (ft_error(NULL, "Map is not of type .cub!"), NULL);
	map_fd = open(map_path, O_RDONLY);
	if (map_fd == -1)
		return (ft_error(NULL, "Map path is invalid!"), NULL);
	map_info = map_info_init();
	if (!map_info || map_info_fill(map_info, map_fd) == false)
	{
		if (map_info)
